...
2. Load the dataset: Run the cell that loads the datset and get the dimensions of the dataset matrix. Optionally, print the dimensions (n,m).
...
6. Initialise weights: Run either the cell containing the function "init_random_weights" or "init_weights_from_data", depending on whether the weights are initialised with random numbers or random samples from the training data. Then call the chosen function by running the appropriate cell. Optionally, you can check whether the weight matrix is normalised.
